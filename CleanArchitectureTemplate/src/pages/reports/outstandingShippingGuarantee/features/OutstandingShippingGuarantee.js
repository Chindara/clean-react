...
export default function OutStandingShippingGuarantee({ searchVisible, setSearchVisible }) {
	...
	const getOutStandingShippingGuarantee = async () => {

		try {
			const response = await ReportService.getOutStandingShippingGuarantee(companyId, startDate, endDate, selectedPurchaseId, searchVisible);
	...
	const {
		...
		queryKey: ['outstandingShippingGuarantee1', { startDate, endDate, selectedPurchaseId, searchVisible }],
		queryFn: () => getOutStandingShippingGuarantee(),
	...
	return (
		<>
			<SearchBar searchVisible={searchVisible} setSearchVisible={setSearchVisible} canClose={false}>
				...
				<ExportExcelButton title='Export to Excel' filename='OutStandingShippingGuarantee' headers={columns.map((x) => x.Header)} data={reportData} />
			</SearchBar>
...
OutStandingShippingGuarantee.propTypes = {
	value: PropTypes.any,
