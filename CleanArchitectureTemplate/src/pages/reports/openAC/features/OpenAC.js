...
export default function OpenAc({ searchVisible, setSearchVisible }) {
	...
	const getOpenAccountPayment = async () => {
		try {
			const response = await ReportService.getOpenAccountPayment(companyId, startDate, endDate, selectedPurchaseId, searchVisible);
	...
	const {
		...
		queryKey: ['openAC', { startDate, endDate, selectedPurchaseId, searchVisible }],
		queryFn: () => getOpenAccountPayment(),
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'Payment Request Date', accessor: 'paymentRequestDate' },
	...
	return (
		<>
			<SearchBar searchVisible={searchVisible} setSearchVisible={setSearchVisible} canClose={false}>
				...
				<ExportExcelButton title='Export to Excel' filename='OpenAccount' headers={columns.map((x) => x.Header)} data={reportData} />
			</SearchBar>

			<SimpleBar>
				<ReactTable data={reportData} columns={columns} paginate={false} isFetching={isFetching} />
			</SimpleBar>
		</>
	);
}
