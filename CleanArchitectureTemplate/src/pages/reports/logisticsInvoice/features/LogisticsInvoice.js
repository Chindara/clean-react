...
export default function LogisticsInvoice({ searchVisible, setSearchVisible }) {
	...
	const getLogisticInvoice = async () => {
		//if (!startDate || !endDate) return { purchaseOrders: [], pagination: {} };

		try {
			const response = await ReportService.getLogisticInvoice(companyId, startDate, endDate, selectedPurchaseId, searchVisible);
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'Shipment Cleared Date', accessor: 'shipmentClearedDate' },
			{ Header: 'Clearing Agent', accessor: 'clearingAgent' },
			{ Header: 'Logistic Invoice No', accessor: 'logisticInvoiceNumber' },
			{ Header: 'Currency', accessor: 'currency' },
			{ Header: 'Payment Category', accessor: 'paymentCategory' },
			...
			{ Header: 'Customs Declaration No', accessor: 'customsDeclarationNumber' },
			{ Header: 'Finance Submitted Date', accessor: 'financeSubmittedDate' },
		...
		<>
			<SearchBar searchVisible={searchVisible} setSearchVisible={setSearchVisible} canClose={false}>
				...
				<DateRangePicker
					...
					size='lg'
				/>
				<ExportExcelButton title='Export to Excel' filename='LogisticsInvoice' headers={columns.map((x) => x.Header)} data={reportData} />
			</SearchBar>
