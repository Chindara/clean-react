...
export default function TotalCost({ searchVisible, setSearchVisible }) {
	...
	const getTotalCost = async () => {
		...
		try {
			const response = await ReportService.getTotalCost(companyId, startDate, endDate, selectedPurchaseId, selectedShipmentId, searchVisible);
	...
	const { data: { reportData = [] } = {}, isFetching } = useQuery({
		queryKey: ['purchaseOrderTT', { startDate, endDate, selectedPurchaseId, selectedShipmentId, searchVisible }],
		queryFn: () => getTotalCost(),
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'Payment Ref', accessor: 'paymentReference' },
