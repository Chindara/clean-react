...
export default function OutstandingTT({ searchVisible, setSearchVisible }) {
	...
	const getOutstandingTT = async () => {

		try {
			const response = await ReportService.getOutstandingTT(companyId, startDate, endDate, selectedPurchaseId, searchVisible);
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'CD Received', accessor: 'cdReceived' },
			{ Header: 'TT Requested Date', accessor: 'ttRequestedDate' },
			{ Header: 'Advance Date', accessor: 'advanceDate' },
			{ Header: 'Payment Remark', accessor: 'paymentRemark' },
		],
		[theme]
	);
