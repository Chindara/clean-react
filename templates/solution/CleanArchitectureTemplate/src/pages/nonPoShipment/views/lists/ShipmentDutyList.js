...
export default function DutyList({ handleDutyForm, shipmentId }) {
	...
	const getDuties = async () => {
		...
		// const temp = await ShipmentDutyService.getByIdNonPO(companyId, shipmentId);

		const { data: items = [] } = await ShipmentDutyService.getAllNonPO(companyId, shipmentId);
		console.log(items);
		return { duties: items };
	};
