...
export default function CustomClearance({ searchVisible, setSearchVisible }) {
	...
	const getCustomClearanceLeadTime = async () => {
		try {
			const response = await ReportService.getCustomClearanceLeadTime(companyId, startDate, endDate, selectedPurchaseId, searchVisible);
	...
	const {
		...
		queryKey: ['customClearance1', { startDate, endDate, selectedPurchaseId, searchVisible }],
		queryFn: () => getCustomClearanceLeadTime(),
	...
	const columns = useMemo(
		() => [
			...
			{ Header: 'Documents To CHA', accessor: 'documentsToCHA' },
			{ Header: 'Cargo Cleared Date', accessor: 'cargoClearedDate' },
			{ Header: 'Shipment Type', accessor: 'shipmentType' },
			{ Header: 'Payment Term', accessor: 'paymentTerm' },
			{ Header: 'Inco Term', accessor: 'incoTerm' },
			{ Header: 'No Of Dates', accessor: 'noOfDates' },
		...
		<>
			<SearchBar searchVisible={searchVisible} setSearchVisible={setSearchVisible} canClose={false}>
				...
				<DateRangePicker
					...
					size='lg'
				/>
				<ExportExcelButton title='Export to Excel' filename='CustomClearanceLeadTime' headers={columns.map((x) => x.Header)} data={reportData} />
			</SearchBar>
