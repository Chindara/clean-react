...
export default function BankFormOA() {
	...
	const handleOnChange = (event) => {
		...
		setPurchaseOrderBank((prevState) => ({ ...prevState, OA: { ...prevState.OA, [name]: value } }));
	};
	...
	const handleDatePickerChange = (property, value) => {
		...
		setPurchaseOrderBank((prevState) => ({ ...prevState, OA: { ...prevState.OA, [property]: value } }));
	};
	...
	const handleAutoCompleteChange = (value) => {
		...
		setPurchaseOrderBank((prevState) => ({ ...prevState, OA: { ...prevState.OA, bank: value } }));
				...
				<Grid item xs={12} sm={6}>
					<Stack spacing={0.5}>
						...
						<Autocomplete
							...
							value={bankOptions.find((option) => option?.value === purchaseOrderBank.OA.bank?.value) || null}
				...
				<Grid item xs={12} sm={6}>
					<Stack spacing={0.5}>
						...
						<TextField
							...
							fullWidth
							size={CONTROL_SIZE}
							value={purchaseOrderBank.OA.bankReference || ''}
					...
					<Stack spacing={0.5}>
						...
						<LocalizationProvider dateAdapter={AdapterDateFns}>
							<DatePicker
								...
								value={purchaseOrderBank.OA.paymentDate}
				...
				<Grid item xs={12} sm={6}>
					<Stack spacing={0.5}>
						...
						<LocalizationProvider dateAdapter={AdapterDateFns}>
							<DatePicker
								inputFormat='dd/MM/yyyy'
								value={purchaseOrderBank.OA.dueDate}
								onChange={(newValue) => handleDatePickerChange('dueDate', newValue)}
								renderInput={(params) => <TextField {...params} />}
							/>
						</LocalizationProvider>
					</Stack>
				...
				<Grid item xs={12} sm={6}>
					<Stack spacing={0.5}>
						...
						<LocalizationProvider dateAdapter={AdapterDateFns}>
							<DatePicker
								inputFormat='dd/MM/yyyy'
								value={purchaseOrderBank.OA.copyDocsSubmittedToFinance}
				...
				<Grid item xs={12} sm={6}>
					<Stack spacing={0.5}>
						...
						<TextField
							...
							fullWidth
							size={CONTROL_SIZE}
							value={purchaseOrderBank.OA.remark || ''}
							onChange={handleOnChange}
						/>
					</Stack>
				</Grid>
